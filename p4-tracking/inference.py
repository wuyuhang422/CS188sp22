...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            map.append((x,y))
    variableDomainsDict[GHOST0] = map[:]
    variableDomainsDict[GHOST1] = map[:]
    variableDomainsDict[PAC] = map[:]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for hvar in eliminationOrder:
            otherfactors, merged = joinFactorsByVariable(allCPTs, hvar)
            if merged.unconditionedVariables() != set([hvar]):
                merged = eliminate(merged, hvar)
            else:
                merged = None
            allCPTs = list(otherfactors)
            if merged:
                allCPTs.append(merged)
        result = joinFactors(allCPTs)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldpos in self.allPositions:
            ...
            for newpos in self.allPositions:
                new_beliefs[newpos] += newposDist[newpos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.particles:
            dict[position] += self.getObservationProb(observation, gameState.getPacmanPosition(), position, self.getJailPosition())
        if dict.total() == 0:
            self.initializeUniformly(gameState)
            return
        dict.normalize()
        self.particles = [dict.sample() for _ in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
